...
const LoadBasket = async () => {
  ...
  if (ctx.userInfo.id) {
    let res = await fetch(
      ctx.rbUrl() + "/api/basket?shop=" + good.value.OwnerShop.Ref,
      {
        method: "GET",
...
const Inc = async () => {
  ...
  let res = await fetch(
    ctx.rbUrl() + "/api/basket/increase?goodref=" + good.value.Ref,
    {
      method: "POST",
      headers: await ctx.authHeadersAppJson(),
    }
...
const Dec = async () => {
  ...
  let res = await fetch(
    ctx.rbUrl() + "/api/basket/decrease?goodref=" + good.value.Ref,
    {
      method: "POST",
      headers: await ctx.authHeadersAppJson(),
    }
...
<template>
  <nav>
    <RouterLink
      v-if="isOwner"
      v-bind:to="`/edit-good/${good.OwnerShop.Ref}/${good.Ref}`"
    >
  ...
  <RouterLink :to="`/shop/${good.OwnerShop.Ref}`"
    >Витрина "{{ good.OwnerShop.Caption }}"</RouterLink
  >
  <div>&nbsp;</div>
  <h1>{{ good.Caption }}</h1>
